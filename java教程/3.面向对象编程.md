...
继承是面向对象编程中非常强大的一种机制，它首先可以复用代码。当我们让`Student`从`Person`继承时间，`Student`就获得了`Person`的所有功能，我们只需要为`Student`编写新增的功能。
...
Java只运行一个class继承自一个类，因此，一个类有且仅有一个父类。只有`Object`特殊，它没有父类。

类似的，如果我们只定义一个继承自`Person`的`Teacher`，它们的继承树关系如下：

![alt text](image-2.png)

**protected**

继承有一个特点，就是子类无法访问父类的`private`字段或者`private`方法。例如，`Student`类就无法访问`Person`类的`name`和`age`字段：
...
因此，`protected`关键字可以把字段和方法的访问权限控制在继承树内部，一个`protected`字段和方法可以被其子类，以及子类的子类所访问，后面我们还会详细访问讲解。

**super**

`super`关键字表示父类(超类)。子类引用父类的字段时，可以用`super.fileName`。例如：
...
public class Main {
    public static void main(String[] args) {
        Student s = new Student("xiao ming", 12, 13);
    }
}
...
这是因为在Java中，任何`class`的构造方法，第一行语句必须是调用父类的构造方法。如果没有明确的调用父类的构造方法，编译器会帮我们自动加一个`super();`，所以，`Student`的构造方法实际上是这样的：
...
因此我们得出结论：如果父类没有默认构造方法，子类就必须显式调用`super()`并给出参数以便让编译器定位到父类的一个合适的构造方法。
...
从Java 15 开始，允许使用`sealed`修饰class，并通过`permits`明确写出该class继承的子类名称。
